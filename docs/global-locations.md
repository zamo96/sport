...
Production therefore requires `LOCATION_PROVIDER_URL` pointing to a commercial or self-hosted Nominatim-compatible endpoint before provider calls are made at all. Without it, worldwide provider calls are disabled and reverse geocoding answers from the packaged catalog instead: `findNearestCatalogCity` returns the closest catalog city within 100 km, reconciled to a legacy place when one matches. That names a city anywhere the catalog has coverage without a geocoder, at the cost of naming the nearest catalogued city rather than the exact locality. A configured provider still takes precedence and remains the accurate path. `LOCATION_PROVIDER_USER_AGENT` should identify the application and a monitored contact address. Provider attribution must be shown by the client according to the selected provider's terms.
