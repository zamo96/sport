...
export function AuthRequiredSheet({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex items-end justify-center bg-ink/45 px-4 pb-6"
      onKeyDown={(event) => {
        if (event.key === "Escape") {
      ...
      <Panel className="w-full max-w-md space-y-4 rounded-[32px] bg-cream">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button
            ...
            className="inline-flex min-h-12 w-full items-center justify-center rounded-2xl bg-ink px-4 text-sm font-semibold text-white shadow-card transition active:scale-[0.985]"
          >
            Продолжить
          </button>
        </div>
