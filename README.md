...
- Яндекс Карты JS API v3 через provider-слой
...
- Swipe deck с `лайк / дизлайк / суперлайк`
- Автоматическое создание мэтча при взаимном лайке
- Список мэтчей и MVP-чат
- Список кортов и карта через Яндекс Карты
...
- Список своих поисков, отклики и выбор одного игрока для игры
...
YANDEX_MAPS_SEARCH_API_KEY=серверный_ключ_поиска_организаций
```

Для страницы спортивных центров можно отдельно включить поиск организаций Яндекса. Для тенниса используется запрос `Санкт-Петербург Теннисный клуб` с пагинацией, чтобы забирать не только первые 50 точек выдачи.
...
- Реальный map provider с кластеризацией кортов и поиском мест
